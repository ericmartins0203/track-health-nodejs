...
describe("Update Address", () => {
  ...
  it("will return status 200 and the updated address as json response", async () => {
    ...
    const updateAddressData = generateAddress();

    const response = await supertest(app)
      .patch(`/address/${createAddress.body.address.id}`)
    ...
    expect(response.body.message).toStrictEqual("Successfully updated address");
  });
  ...
  it("will return status 400 and Address not found", async () => {
    ...
    const updateAddress = generateAddress();

    const response = await supertest(app)
      .patch("/address/wrongId")
      .send(updateAddress)
      .set("Authorization", `Bearer ${token}`);
